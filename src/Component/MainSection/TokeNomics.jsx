import { useViewport } from "react-viewport-hooks";

function TokenNomics() {
  const { vw } = useViewport();
  return (
    <div className="bg-white h-max rounded-lg mt-4 p-6">
      <div className="text-[#0F1629] text-2xl font-semibold">Tokenomics</div>
      <div className="my-6">
        <div className="text-[#1D1D1D] text-xl font-semibold">
          Initial Distribution
        </div>
        <div className="py-4 flex max-[476px]:flex-col ">
          <div>
            <svg
              width={vw <= 576 ? "100" : "179"}
              height="180"
              viewBox="0 0 179 180"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <circle
                cx="89.5"
                cy="89.7969"
                r="74.5"
                stroke="#0082FF"
                strokeWidth="30"
              ></circle>
              <mask id="path-2-inside-1_1_922" fill="white">
                <path d="M89.5 0.296875C74.3468 0.296875 59.4417 4.14426 46.1817 11.4784C32.9217 18.8126 21.7409 29.3935 13.6873 42.2293C5.63363 55.065 0.970851 69.7355 0.135952 84.8657C-0.698948 99.9958 2.32137 115.09 8.91382 128.734L89.5 89.7969V0.296875Z"></path>
              </mask>
              <path
                d="M89.5 0.296875C74.3468 0.296875 59.4417 4.14426 46.1817 11.4784C32.9217 18.8126 21.7409 29.3935 13.6873 42.2293C5.63363 55.065 0.970851 69.7355 0.135952 84.8657C-0.698948 99.9958 2.32137 115.09 8.91382 128.734L89.5 89.7969V0.296875Z"
                stroke="#FAA002"
                strokeWidth="60"
                mask="url(#path-2-inside-1_1_922)"
              ></path>
              <circle cx="89.5" cy="89.5" r="59.5" fill="white"></circle>
              <circle cx="89.5" cy="89.5" r="59.5" stroke="#0082FF"></circle>
            </svg>
          </div>
          <div className="flex flex-col justify-center ml-8 max-[476px]:ml-0">
            <div className="flex items-center">
              <svg
                width="12"
                height="13"
                viewBox="0 0 12 13"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <rect
                  y="0.796875"
                  width="12"
                  height="12"
                  rx="6"
                  fill="#0082FF"
                ></rect>
              </svg>
              <div className="text-[#202020] min-[576px]:text-lg font-medium ml-4">
                Crowdsale investors: 80%
              </div>
            </div>
            <div className="flex items-center pt-2">
              <svg
                width="12"
                height="13"
                viewBox="0 0 12 13"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <rect
                  y="0.796875"
                  width="12"
                  height="12"
                  rx="6"
                  fill="#FAA002"
                ></rect>
              </svg>
              <div className="text-[#202020] mins-[576px]:text-lg font-medium ml-4">
                Foundation: 20%
              </div>
            </div>
          </div>
        </div>
        <div className="text-[#3E424A] text-sm py-2">
          Lorem ipsum dolor sit amet consectetur. Cras aliquet tristique ornare
          vestibulum nunc dignissim vel consequat. Leo etiam nascetur bibendum
          amet enim sit eget leo amet. At metus orci augue fusce eleifend lectus
          eu fusce adipiscing. Volutpat ultrices nibh sodales massa habitasse
          urna felis augue. Gravida aliquam fermentum augue eu. Imperdiet
          bibendum amet aliquam donec. Eget justo dui metus odio rutrum. Vel
          ipsum eget in at curabitur sem posuere facilisis vitae. Sed lorem sit
          mauris id eget arcu ut. Vulputate ipsum aliquet odio nisi eu ac risus.
        </div>
      </div>
    </div>
  );
}

export default TokenNomics;
